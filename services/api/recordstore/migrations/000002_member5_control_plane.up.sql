...
CREATE TABLE voice_sessions (
    ...
    CONSTRAINT voice_sessions_timestamps_valid CHECK (
        ...
        OR (status = 'ended' AND started_at IS NOT NULL AND ended_at IS NOT NULL AND ended_at >= started_at AND failure_error_code IS NULL)
...
CREATE TABLE voice_session_start_requests (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (account_id, idempotency_key),
    CONSTRAINT voice_session_start_requests_key_not_empty CHECK (idempotency_key <> ''),
    CONSTRAINT voice_session_start_requests_hash_not_empty CHECK (request_hash <> ''),
    CONSTRAINT voice_session_start_requests_session_key
        FOREIGN KEY (session_id, account_id)
        REFERENCES voice_sessions (id, account_id)
        ON DELETE RESTRICT
);
